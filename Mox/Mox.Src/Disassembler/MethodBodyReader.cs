...
{
    static class ILGeneratorExtensionMethods
    {
    ...
    class MethodBodyReader
    {
        ...
        MethodBodyReader(DynamicMethod method, Type[] arguments = null, IList<LocalVariableInfo> locals = null)
        {
            this.method = method;

            var ilGenerator = method.GetILGenerator();
            if (ilGenerator == null)
                throw new ArgumentException("Method has no IL generator");
            ...
            this.parameters = method.GetParameters();
            this.locals = locals;
            this.module = new DynamicModule(ilGenerator);
            this.il = new ByteBuffer(bytes);
            this.instructions = new List<Instruction>((bytes.Length + 1) / 2);
        }
        ...
        public static List<Instruction> GetInstructions(DynamicMethod method, Type[] arguments = null, IList<LocalVariableInfo> locals = null)
        {
            var reader = new MethodBodyReader(method, arguments, locals);
            reader.ReadInstructions();
            return reader.instructions;
        }
